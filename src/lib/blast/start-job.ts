    ...
    async (tx) => {
      ...
      const job = await tx.blastJob.create({
        data: {
          ...
          // The unified block is snapshotted verbatim, so a later admin edit
          // cannot change what an in-flight job sends (RULES.md §11).
          snapshotImage1Key: campaign.image1Key,
          snapshotImage1Mime: campaign.image1Mime,
          snapshotImage2Key: campaign.image2Key,
          snapshotImage2Mime: campaign.image2Mime,
          snapshotButtons: campaign.buttons ?? Prisma.DbNull,
