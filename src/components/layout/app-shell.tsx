 ...
 *
 * Brutalist treatment: heavy 4px black rules separate every structural region,
 * the active nav item is a fully inverted block rather than a tinted pill, and
 * all surfaces are opaque (no translucency, no backdrop blur).
 *
 * Motion: the drawer slides, the active-nav indicator animates as a shared
 * layout element, and route content fades in keyed on the pathname so
...
function ShellBrand({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="flex items-center gap-3">
      <span
        aria-hidden="true"
        className="inline-flex size-10 shrink-0 items-center justify-center border-4 border-black bg-primary"
      >
      ...
      {items.map((item) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "group relative flex min-h-11 items-center gap-3 border-2 border-black px-3 text-sm font-black uppercase tracking-wide",
  ...
  return (
    <div className="mt-6 space-y-3 border-t-4 border-black pt-4">
      <div className="flex items-center gap-3">
        <span
          aria-hidden="true"
          className="inline-flex size-10 shrink-0 items-center justify-center border-4 border-black bg-foreground text-xs font-black text-background"
        >
...
export function AppShell({
  ...
  return (
    <div className="relative flex min-h-dvh flex-col lg:flex-row">
      ...
      {/* Desktop sidebar */}
      <aside className="sticky top-0 hidden h-dvh w-64 shrink-0 flex-col border-r-4 border-black bg-sidebar p-4 lg:flex xl:w-72">
        <ShellBrand title={title} subtitle={subtitle} />
        <div className="mt-6 flex-1 overflow-y-auto border-t-4 border-black pt-4">
          <NavLinks
            items={items}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="sticky top-0 z-30 flex min-h-14 items-center gap-3 border-b-4 border-black bg-background px-3 lg:hidden">
          <Button
            variant="ghost"
            size="icon"
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-[11px] font-bold uppercase text-foreground">
              {title}
            </p>
          </div>
          <form action={signOutAction}>
          ...
          {open ? (
            ...
            >
              <button
                ...
                className="absolute inset-0 overlay-forest"
              ...
              <motion.div
                ...
                className="absolute inset-y-0 left-0 flex w-[min(19rem,86vw)] flex-col border-r-4 border-black bg-sidebar p-4 shadow-lift"
                initial={{ x: "-100%" }}
                animate={{ x: 0 }}
                exit={{ x: "-100%" }}
        ...
        <nav
          aria-label="Primary"
          className="fixed inset-x-0 bottom-0 z-30 border-t-4 border-black bg-background pb-[env(safe-area-inset-bottom)] lg:hidden"
        >
          <ul className="mx-auto flex max-w-2xl items-stretch">
            {tabs.map((item) => {
              const active = isActive(item.href);
              return (
                <li key={item.href} className="flex-1 border-r-2 border-black">
                  <Link
                    href={item.href}
                    aria-current={active ? "page" : undefined}
                    className={cn(
                      "flex min-h-14 flex-col items-center justify-center gap-1 px-1 py-2 text-[11px] font-black uppercase",
                      "transition-colors duration-100 [transition-timing-function:steps(2,end)]",
            ...
            <li className="flex-1">
              <button
                ...
                aria-expanded={open}
                aria-controls="shell-drawer"
                className="flex min-h-14 w-full flex-col items-center justify-center gap-1 px-1 py-2 text-[11px] font-black uppercase text-foreground transition-colors duration-100 [transition-timing-function:steps(2,end)] active:bg-accent focus-visible:outline-4 focus-visible:-outline-offset-4 focus-visible:outline-ring"
              >
